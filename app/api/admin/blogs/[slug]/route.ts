...
export const PUT = withAdmin<Params>(
  async (req, { params, user, site }) => {
    const { slug } = await params;

    const parsed = blogInputSchema
      .partial()
    ...
    const nextAuthorId =
      parsed.data.authorId === undefined
    ...
    if (parsed.data.status === 'publish' && !existing.publishedAt) {
